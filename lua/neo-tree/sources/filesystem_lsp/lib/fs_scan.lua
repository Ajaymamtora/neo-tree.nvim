...
M.get_items = function(state, parent_id, path_to_reveal, callback, async, recursive)
  ...
  -- If we're at the root level, create workspace root nodes
  if not parent_id then
    context.root = {
      id = "workspaces",
      name = "Workspaces",
    ...
    for _, workspace_root in ipairs(state.workspace_roots) do
      local workspace_node = file_items.create_item(context, workspace_root, "directory")
      workspace_node.name = vim.fn.fnamemodify(workspace_root, ":t")
      workspace_node.loaded = false
      table.insert(context.root.children, workspace_node)
    end

    context.folders[context.root.id] = context.root
    state.default_expanded_nodes = { context.root.id }
  else
    -- Create root folder for the specific workspace or subdirectory
    context.root = file_items.create_item(context, parent_id, "directory")
    context.root.name = vim.fn.fnamemodify(context.root.path, ":~")
    context.root.loaded = true
    context.folders[context.root.path] = context.root
    state.default_expanded_nodes = state.force_open_folders or { state.path }
  end

  context.root.search_pattern = state.search_pattern
...
local function handle_search_pattern(context)
  local state = context.state
  local search_opts = {
    filtered_items = state.filtered_items,
    find_command = state.find_command,
...
local function handle_refresh_or_up(context, async)
  local parent_id = context.parent_id
  local path_to_reveal = context.path_to_reveal
  local state = context.state
  local path = parent_id or state.path
  ...
  if parent_id == nil then
    -- We're at the root level, so we need to load all workspace roots
    for _, workspace_root in ipairs(state.workspace_roots) do
      table.insert(context.paths_to_load, workspace_root)
    end
  else
    -- We're inside a specific workspace or subdirectory
    if utils.truthy(state.force_open_folders) then
      for _, f in ipairs(state.force_open_folders) do
        table.insert(context.paths_to_load, f)
      end
...
local function job_complete(context)
  local state = context.state
  local parent_id = context.parent_id

  if context.root and context.root.children then
    file_items.advanced_sort(context.root.children, state)
  end

  if parent_id then
    -- lazy loading a child folder
    renderer.show_nodes(context.root.children, state, parent_id, context.callback)
  else
    -- full render of the tree
    renderer.show_nodes({ context.root }, state, nil, context.callback)
  end

...
local function get_workspace_roots()
  ...
  for _, client in pairs(lsp.get_active_clients()) do
    for _, root in ipairs(client.config.workspace_folders or {}) do
      roots[root.name] = root.name
    end
  end
...
M.follow = function(callback, force_show)
  ...
  utils.debounce("neo-tree-follow", function()
    return M.follow_internal(callback, force_show)
  end, 100, utils.debounce_strategy.CALL_LAST_ONLY)
end

M.follow_internal = function(callback, force_show, async)
  log.trace("follow called")
  ...
  -- Find the appropriate workspace root for the current file
  local current_workspace_root = nil
  for _, root in ipairs(state.workspace_roots) do
    if utils.is_subpath(root, path_to_reveal) then
      current_workspace_root = root
      break
    end
  end

  if not current_workspace_root then
    log.debug("Current file is not in any workspace root")
    return false
  end

  -- Check if we need to switch to a different workspace root
  if current_workspace_root ~= state.path then
    log.debug("Switching to workspace root: " .. current_workspace_root)
    state.path = current_workspace_root
    -- You might want to trigger a full refresh of the tree here
    M.navigate(state, current_workspace_root, path_to_reveal, callback, async)
    return true
  end
  ...
  require("neo-tree.sources.filesystem.lib.fs_scan").get_items(
    state,
    nil,
    path_to_reveal,
    function()
...
M.navigate = function(state, path, path_to_reveal, callback, async)
  ...
  -- Get workspace roots if not already set
  if not state.workspace_roots then
    state.workspace_roots = get_workspace_roots()
    if #state.workspace_roots == 0 then
      state.workspace_roots = { vim.fn.getcwd() }
    end
  end

  -- If path is not provided, use the first workspace root
  if not path then
    path = state.workspace_roots[1]
  end

  -- Check if the path is one of the workspace roots
  local is_workspace_root = false
  for _, root in ipairs(state.workspace_roots) do
    if root == path then
  ...
  -- If it's not a workspace root, find the appropriate workspace
  if not is_workspace_root then
    for _, root in ipairs(state.workspace_roots) do
      if utils.is_subpath(root, path) then
        state.path = root
        break
      end
    end
  ...
  local function navigate_internal()
    state.dirty = false
    local is_search = utils.truthy(state.search_pattern)
    ...
    if path_to_reveal then
      ...
      local follow_file = state.follow_current_file.enabled
        and not is_search
        and not is_current
        and M.get_path_to_reveal()

      local handled = false
      if utils.truthy(follow_file) then
        handled = M.follow_internal(callback, true, async)
      end
    ...
    local config = require("neo-tree").config
    if config.enable_git_status and not is_search and config.git_status_async then
      require("neo-tree.git").status_async(
        state.path,
        state.git_base,
        config.git_status_async_options
      )
    end
  end
  ...
  -- Create root nodes for all workspace roots
  state.root_nodes = {}
  for _, root in ipairs(state.workspace_roots) do
    local root_node = {
      id = root,
    ...
    table.insert(state.root_nodes, root_node)
  end

  -- Trigger a redraw
  renderer.show_nodes(state.root_nodes, state)
end

M.reset_search = function(state, refresh, open_current_node)
  state.workspace_roots = get_workspace_roots()
  if #state.workspace_roots == 0 then
    -- Fall back to current working directory if no workspace roots
    state.workspace_roots = { vim.fn.getcwd() }
  end

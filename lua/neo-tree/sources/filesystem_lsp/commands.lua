...
M.switch_workspace = function(state)
  local roots = state.workspace_roots or get_workspace_roots()
  vim.ui.select(roots, {
    ...
    if choice then
      state.path = choice
      require("neo-tree.sources.filesystem_lsp").navigate(state, choice)
    end
  end)
end
